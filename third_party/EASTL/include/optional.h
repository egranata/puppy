...
namespace std
{
	...
	namespace Internal
	{
		...
		/// optional_storage
		///
		template<typename T, bool IsTriviallyDestructible = std::is_trivially_destructible_v<T>>
		struct optional_storage
		{
			...
			// The std::optional<T> type will set the empty_val in this case to "initialize" its member data.
			union
			{
	...
	public:
		...
		inline EA_CONSTEXPR optional(value_type&& value) EA_NOEXCEPT_IF(std::is_nothrow_move_constructible_v<T>)
		...
		{
			construct_value(std::move(T(std::forward<Args>(args)...)));
			engaged = true;
		}
		...
		{
			construct_value(std::move(T(ilist, std::forward<Args>(args)...)));
			engaged = true;
		}
