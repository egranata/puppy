...
namespace std
{
	...
	template <typename T> 
	inline void swap(T& a, T& b) EA_NOEXCEPT_IF(std::is_nothrow_move_constructible<T>::value && std::is_nothrow_move_assignable<T>::value)
	{
		T temp(EASTL_MOVE(a));  // EASTL_MOVE uses std::move when available, else is a no-op.
		a = EASTL_MOVE(b);
		b = EASTL_MOVE(temp);
	}
	...
	///
		...
		// We declare this version of 'std::swap' to make compile-time existance checks for swap functions possible.  
		//
		#if EASTL_VARIADIC_TEMPLATES_ENABLED
			std::unused swap(std::argument_sink, std::argument_sink);
		#else
		...
		template <typename T>
		struct is_swappable
			: public integral_constant<bool, !std::is_same<decltype(swap(std::declval<T&>(), std::declval<T&>())), std::unused>::value> {}; // Don't prefix swap with std:: as we want to allow user-defined swaps via argument-dependent lookup.
	#endif
	...
	struct is_nothrow_swappable_helper_noexcept_wrapper
		{ const static bool value = noexcept(swap(std::declval<T&>(), std::declval<T&>())); };

	template <typename T, bool>
	struct is_nothrow_swappable_helper
		: public std::integral_constant<bool, is_nothrow_swappable_helper_noexcept_wrapper<T>::value> {}; // Don't prefix swap with std:: as we want to allow user-defined swaps via argument-dependent lookup.
	...
	struct is_nothrow_swappable
		: public std::is_nothrow_swappable_helper<T, std::is_swappable<T>::value> {};
	...
	///
	///
	template <typename T, typename U, bool OneTypeIsVoid = (std::is_void<T>::value || std::is_void<U>::value)>
	struct is_swappable_with_helper
	{
		// Don't prefix swap with std:: as we want to allow user-defined swaps via argument-dependent lookup.
	    static const bool value =
	        !std::is_same<decltype(swap(std::declval<T>(), std::declval<U>())), std::unused>::value &&
	        !std::is_same<decltype(swap(std::declval<U>(), std::declval<T>())), std::unused>::value;
    };
	...
	struct is_swappable_with 
			: public std::bool_constant<is_swappable_with_helper<T, U>::value> {}; 
    ...
    template <typename T, typename U>
    struct is_nothrow_swappable_with : public std::bool_constant<is_nothrow_swappable_with_helper<T, U>::value> {};
	...
	struct iter_swap_impl
	{
		...
		{
			typedef typename std::iterator_traits<ForwardIterator1>::value_type value_type_a;

			value_type_a temp(EASTL_MOVE(*a)); // EASTL_MOVE uses std::move when available, else is a no-op.
			*a = EASTL_MOVE(*b);
			*b = EASTL_MOVE(temp); 
		}
	};
	...
	struct iter_swap_impl<true>
	{
		...
		static void iter_swap(ForwardIterator1 a, ForwardIterator2 b)
		{
			swap(*a, *b);  // Don't prefix swap with std:: as we want to allow user-defined swaps via argument-dependent lookup.
		}
	};
	...
	{
		typedef typename std::iterator_traits<ForwardIterator1>::value_type value_type_a;
		typedef typename std::iterator_traits<ForwardIterator2>::value_type value_type_b;
		typedef typename std::iterator_traits<ForwardIterator1>::reference  reference_a;
		typedef typename std::iterator_traits<ForwardIterator2>::reference  reference_b;

		std::iter_swap_impl<std::type_and<std::is_same<value_type_a, value_type_b>::value, std::is_same<value_type_a&, reference_a>::value, std::is_same<value_type_b&, reference_b>::value >::value >::iter_swap(a, b);
	}
	...
	swap_ranges(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2)
	{
		for(; first1 != last1; ++first1, ++first2)
			iter_swap(first1, first2); // Don't prefix swap with std:: as we want to allow user-defined swaps via argument-dependent lookup.
		return first2;
	}
	...
	inline void
	swap(T (&a)[N], T (&b)[N]) EA_NOEXCEPT_IF(std::is_nothrow_swappable<T>::value)
	{
	...
	///
	template <class T>
	EA_CONSTEXPR typename std::add_const<T>::type& as_const(T& t) EA_NOEXCEPT
		{ return t; }

	// The C++17 forbids 'std::as_const' from accepting rvalues.  Passing an rvalue reference to 'std::as_const'
	...
	struct pair
	{
		...
		template <typename TT1 = T1,
		          typename TT2 = T2,
		          class = std::enable_if_t<std::is_default_constructible_v<TT1> &&
		                                     std::is_default_constructible_v<TT2>>>
		EA_CONSTEXPR pair()
		    : first(), second()
		{
		}
		...
		template <typename TT2 = T2, typename = std::enable_if_t<std::is_default_constructible_v<TT2>>>
		...
		//	
		// error: 'std::pair<T1, T2>::pair(T1&&) [with T1 = const int&; T2 = const int&]' cannot be overloaded	
		// error: with 'std::pair<T1, T2>::pair(const T1&) [with T1 = const int&; T2 = const int&]'
		#if !defined(EA_COMPILER_GNUC)
			template <typename TT2 = T2, typename = std::enable_if_t<std::is_default_constructible_v<TT2>>>
			EA_CPP14_CONSTEXPR pair(T1&& x)
				: first(std::move(x)), second()
			{
			}
		#endif
	#endif
	...
	// https://bugs.llvm.org/show_bug.cgi?id=38374
		...
		template<typename T>
		using single_pair_ctor_sfinae = std::enable_if_t<std::is_default_constructible_v<T>>;
	#else
		template<typename>
		...
		template <
		    typename TT1 = T1,
		    typename TT2 = T2,
		    class = std::enable_if_t<std::is_copy_constructible_v<TT1> && std::is_copy_constructible_v<TT2>>>
		...
		template <
		    typename U,
		    typename V,
		    class = std::enable_if_t<std::is_convertible_v<const U&, T1> && std::is_convertible_v<const V&, T2>>>
		...
		template <typename U,
		          typename V,
		          typename = std::enable_if_t<std::is_convertible_v<U, T1> && std::is_convertible_v<V, T2>>>
		EA_CPP14_CONSTEXPR pair(U&& u, V&& v)
		    : first(std::forward<U>(u)), second(std::forward<V>(v))
		{
		}

		template <typename U, typename = std::enable_if_t<std::is_convertible_v<U, T1>>>
		EA_CPP14_CONSTEXPR pair(U&& x, const T2& y)
			: first(std::forward<U>(x)), second(y)
		{
		}

		template <typename V, typename = std::enable_if_t<std::is_convertible_v<V, T2>>>
		...
		template <class... Args1,
		          class... Args2,
		          typename = std::enable_if_t<std::is_constructible_v<first_type, Args1&&...> &&
		                                        std::is_constructible_v<second_type, Args2&&...>>>
		pair(std::piecewise_construct_t pwc, std::tuple<Args1...> first_args, std::tuple<Args2...> second_args)
		    : pair(pwc,
		           std::move(first_args),
	...
	public:
		pair& operator=(const pair& p)
		    EA_NOEXCEPT_IF(std::is_nothrow_copy_assignable_v<T1>&& std::is_nothrow_copy_assignable_v<T2>)
		{
			first = p.first;
			second = p.second;
			return *this;
		...
		pair& operator=(pair&& p)
		    EA_NOEXCEPT_IF(std::is_nothrow_move_assignable_v<T1>&& std::is_nothrow_move_assignable_v<T2>)
		{
		...
		void swap(pair& p) EA_NOEXCEPT_IF(std::is_nothrow_swappable_v<T1>&& std::is_nothrow_swappable_v<T2>)
		{
			std::iter_swap(&first, &p.first);
			std::iter_swap(&second, &p.second);
		}
	};
	...
	///
	template <typename T1, typename T2>
	EA_CPP14_CONSTEXPR inline pair<typename std::remove_reference_wrapper<typename std::decay<T1>::type>::type, 
								   typename std::remove_reference_wrapper<typename std::decay<T2>::type>::type>
	make_pair(T1&& a, T2&& b)
	{
		typedef typename std::remove_reference_wrapper<typename std::decay<T1>::type>::type T1Type;
		typedef typename std::remove_reference_wrapper<typename std::decay<T2>::type>::type T2Type;

		return std::pair<T1Type, T2Type>(std::forward<T1>(a), std::forward<T2>(b));
	}


	// Without the following, VC++ fails to compile code like this: pair<const char*, int> p = std::make_pair<const char*, int>("hello", 0);
	...
	// are of type char const(&)[].
		...
		EA_CPP14_CONSTEXPR inline pair<T1, T2> make_pair(
			const T1& a,
			const T2& b,
			typename std::enable_if<!std::is_array<T1>::value && !std::is_array<T2>::value>::type* = 0)
		{
