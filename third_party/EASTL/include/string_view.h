...
///////////////////////////////////////////////////////////////////////////////
// This file implements the std::string_view which is part of the C++ standard
...
namespace std
{
	...
	public:
		...
		size_type copy(T* pDestination, size_type count, size_type pos = 0) const
		{
			...
			count = std::min(count, mnCount - pos);
		...
		static EA_CPP14_CONSTEXPR int compare(const T* pBegin1, const T* pEnd1, const T* pBegin2, const T* pEnd2)
		{
			...
			const ptrdiff_t nMin = std::min_alt(n1, n2);
		...
		EA_CPP14_CONSTEXPR size_type rfind(T c, size_type pos = npos) const EA_NOEXCEPT
		{
			...
			{
				const value_type* const pEnd = mpBegin + std::min_alt(mnCount - 1, pos) + 1;
		...
		EA_CPP14_CONSTEXPR size_type rfind(const T* s, size_type pos, size_type n) const
		{
			...
			if (EASTL_LIKELY(n <= mnCount))
			{
				if (EASTL_LIKELY(n))
				{
					const const_iterator pEnd = mpBegin + std::min_alt(mnCount - n, pos) + n;
					...
					return std::min_alt(mnCount, pos);
			}
			return npos;
		}
