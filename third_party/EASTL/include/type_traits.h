...
namespace std
{
	...
	template <typename T> struct is_const_value                    : public std::false_type{};
	template <typename T> struct is_const_value<const T*>          : public std::true_type{};
	template <typename T> struct is_const_value<const volatile T*> : public std::true_type{};

	template <typename T> struct is_const : public std::is_const_value<T*>{};
	template <typename T> struct is_const<T&> : public std::false_type{}; // Note here that T is const, not the reference to T. So is_const is false. See section 8.3.2p1 of the C++ standard.
	...
	template <typename T> struct is_volatile_value                    : public std::false_type{};
	template <typename T> struct is_volatile_value<volatile T*>       : public std::true_type{};
	template <typename T> struct is_volatile_value<const volatile T*> : public std::true_type{};

	template <typename T> struct is_volatile : public std::is_volatile_value<T*>{};
	template <typename T> struct is_volatile<T&> : public std::false_type{}; // Note here that T is volatile, not the reference to T. So is_const is false. See section 8.3.2p1 of the C++ standard.
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> std::yes_type is_function_decayed_helper(T*);
		template <typename T> std::no_type  is_function_decayed_helper(...);
		template <typename T> T&  return_T_reference();

		template <typename T, bool = std::is_class<T>::value     || // I don't see why an is_class check here is necessary.
									 std::is_union<T>::value     || // I don't see why an is_union check here is necessary.
									 std::is_void<T>::value      ||
									 std::is_reference<T>::value ||
									 std::is_null_pointer<T>::value >
		struct is_function_helper
			: public integral_constant<bool, sizeof(is_function_decayed_helper<T>(return_T_reference<T>())) == sizeof(std::yes_type)> {};
		...
		template <typename R> struct is_function_ptr_helper : public std::false_type{};
		template <typename R> struct is_function_ptr_helper<R (*)()> : public std::true_type{};
		template <typename R, typename Arg0> struct is_function_ptr_helper<R (*)(Arg0)> : public std::true_type{};
		template <typename R, typename Arg0, typename Arg1> struct is_function_ptr_helper<R (*)(Arg0, Arg1)> : public std::true_type{};
		template <typename R, typename Arg0, typename Arg1, typename Arg2> struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2)> : public std::true_type{};
		template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3> struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3)> : public std::true_type{};
		template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4> struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4)> : public std::true_type{};
		template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4, typename Arg5> struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4, Arg5)> : public std::true_type{};
		template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4, typename Arg5, typename Arg6> struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4, Arg5, Arg6)> : public std::true_type{};
		template <typename R, typename Arg0, typename Arg1, typename Arg2, typename Arg3, typename Arg4, typename Arg5, typename Arg6, typename Arg7> struct is_function_ptr_helper<R (*)(Arg0, Arg1, Arg2, Arg3, Arg4, Arg5, Arg6, Arg7)> : public std::true_type{};

		template <bool is_ref = true>
		struct is_function_chooser
			{ template <typename T> struct result_ : public std::false_type{}; };

		template <>
		struct is_function_chooser<false>
			{ template <typename T> struct result_ : public std::is_function_ptr_helper<T*>{}; };

		template <typename T>
		struct is_function_value
			: public std::is_function_chooser<std::is_reference<T>::value>::template result_<T>{};

		template <typename T>
		struct is_function
			: public std::integral_constant<bool, is_function_value<T>::value>{};
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> struct add_rvalue_reference                      { typedef T&& type;                 }; // Dinkumware has this as { typedef typename std::remove_reference<T>::type&& type; }, but that doesn't seem right to me.
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T>
		typename std::add_lvalue_reference<T>::type declval() EA_NOEXCEPT;
	#else
