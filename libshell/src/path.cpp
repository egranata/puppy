...
namespace {
    void splitList(const char* list, eastl::vector<eastl::string>& output) {
        output.clear();
        if (list == nullptr || list[0] == 0) return;

        eastl::string s_list(list);
        while(true) {
...
namespace libShellSupport {
    const char* findInPotentialPaths(const char* program, const char* list) {
        ...
        if (program_s.find("./") != eastl::string::npos) return strdup(program);
        if (program_s.back() == '/') return strdup(program);

        // plain executable names get searched across candidates
        eastl::vector<eastl::string> path_candidates;
