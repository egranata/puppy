...
def genCommonHeader(table):
    f1 = os.path.join(NEWLIB_INCLUDE_DIR, "syscalls.h")
    with open(f1, "w") as f:
        printLicense(f)
        f.write("#ifndef NEWLIB_SYSCALLS\n")
        f.write("#define NEWLIB_SYSCALLS\n")
        f.write("\n")
...
def genCommonImpl(table):
    f1 = os.path.join(NEWLIB_SYSCALLS_DIR, "syscalls.cpp")
    with open(f1, "w") as f:
        printLicense(f)        
        f.write("#include <newlib/syscalls.h>\n")
        f.write("\n")
